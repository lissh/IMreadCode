...
CID = "f03811f9da177af413accebe96575ad2"
Alias = '请输入别名'
...
# 通知透传模板动作内容
def NotificationTemplateDemo(DeviceType='ios',begin="2016-08-17 17:40:22",end="2016-08-19 17:47:22",Content=u"请填入透传内容",title=u"请填入通知标题",text=u"请填入通知内容",logo="icon.png",logoURL=""):
...
def TransmissionTemplateDemo():
    ...
    template.appId = APPID
    template.appKey = APPKEY
    template.transmissionContent = '这只是个测试，收到告知小青2'
    # iOS 推送需要的PushInfo字段 前三项必填，后四项可以填空字符串
    # template.setPushInfo(actionLocKey, badge, message, sound, payload, locKey, locArgs, launchImage)
...
# APN简单推送
    ...
    alertMsg.alertMsg = '这只是个测试，收到告知小青2'
    apn = APNPayload();
